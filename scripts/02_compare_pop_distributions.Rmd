...
- Bird Island represents ~60% of the South Georgia population (Mackley et al 2025 - census study)
...
pop_dists_norm <- map2(pop_dists_zero, pop_sums, ~.x / .y)
...
**Sources:**
ALL from IUCN RED LIST ASSESSMENT https://www.iucnredlist.org/species/22698305/132640680#population
...
# PEI = Prince Edward + Marion, % of global pop in these two breeding sites
...
We use a "clipped" approach where other population distributions are masked to only include cells where South Georgia birds occur. This prevents artifactual high percentages in cells where South Georgia has effectively zero density but still nonzero very low raster values where other populations are not present (which would incorrectly 100% SG birds where there are basically none).
...
global(pop_dists_norm$south_georgia, fun = c("min", "max"), na.rm = TRUE)  
...
# Matches style used in script 01: % of population, YlGnBu palette, cells < 0.1% masked
...
```{r map-total-global, fig.width=12, fig.height=6}
# Convert to % of population, mask cells < 0.1% — matches script 01 style
total_pct <- total_bird_density / global(total_bird_density, fun = "sum", na.rm = TRUE)[1,1] * 100
total_pct[total_pct < 0.1] <- NA

total_df <- as.data.frame(total_pct, xy = TRUE, na.rm = TRUE)
names(total_df)[3] <- "pct"

p_total_global <- ggplot(total_df, aes(x = x, y = y, fill = pct)) +
  geom_raster() +
  geom_sf(data = ne_land, inherit.aes = FALSE, fill = "grey80", color = NA) +
  ...
  labs(
    title = "Wandering Albatross: Total Global Distribution",
    subtitle = "All populations combined, weighted by breeding pair proportions (Carneiro et al. 2020)"
  ) +
  theme_minimal() +
  theme(
...
ggsave(here("output/maps/total_global_distribution.png"),
       p_total_global, width = 12, height = 6, dpi = 300)
```
