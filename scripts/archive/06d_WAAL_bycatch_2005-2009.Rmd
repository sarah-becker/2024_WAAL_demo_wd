...
# Historical mitigation status for 2005-2009 — approaching current, not yet at current levels
# PLACEHOLDERS: CCAMLR near full; several DLL fleets partial; some PLL beginning; update from literature
hist_fleet_mitigation <- tribble(
  ~fishery, ~fleet,                           ~status,    ~compliance,  ~notes,
  "DLL",    "CCAMLR",                         "full",     0.80,         "near-full compliance, post-2001 measures",
  "DLL",    "Argentina",                      "partial",  0.40,         "partial adoption",
  "DLL",    "Chile",                          "partial",  0.30,         "early adoption",
  "DLL",    "Falklands",                      "full",     0.70,         "good compliance",
  "DLL",    "South Africa",                   "partial",  0.60,         "improving measures",
  "DLL",    "New Zealand",                    "partial",  0.65,         "ACAP signatory measures developing",
  "PLL",    "Japan",                          "partial",  0.30,         "some RFMO requirements beginning",
  "PLL",    "Taiwan",                         "none",     0.0,          "no mitigation",
  "PLL",    "Korea",                          "none",     0.0,          "no mitigation",
  "PLL",    "Spain",                          "partial",  0.30,         "early EU measures",
  "PLL",    "Portugal",                       "partial",  0.30,         "early EU measures",
  ...
  "PLL",    "USSR",                           "none",     0.0,          "historical, no mitigation",
  "PLL",    "USA",                            "partial",  0.40,         "early NMFS requirements",
...
# === SURVIVAL RATES (stage-specific, from Pardo shared data) ===
# Stage-specific survival: s_Juv for juveniles, individual rates for each adult/immature stage
# Behavioral rates (b_*, r_*, k_*) retain full stage-specific structure
surv_params <- list(
  s_Juv = calc_beta_params(0.846, 0.770, 0.910),  # juvenile survival
  s_EF  = calc_beta_params(0.913, 0.875, 0.945),  # experienced breeder failed
  s_ENB = calc_beta_params(0.943, 0.910, 0.968),  # experienced non-breeder
  s_ES  = calc_beta_params(0.895, 0.855, 0.930),  # experienced breeder successful
  s_IF  = calc_beta_params(0.920, 0.882, 0.950),  # inexperienced breeder failed
  s_Imm = calc_beta_params(0.921, 0.883, 0.951),  # immature
  s_IS  = calc_beta_params(0.892, 0.851, 0.928),  # inexperienced breeder successful
  s_PF  = calc_beta_params(0.935, 0.898, 0.963)   # pre-breeder failed
)
...
for (k in seq_along(mitigation_scenarios$efficacy)) {
  ...
  for (j in 1:n_sims) {
    ...
    # Survival: draw each adult/immature stage independently, then draw juvenile
    # Hierarchy: s_Juv < min(all adult stages)
    ...
    # Enforce s_Juv < min(all adult stages): re-draw until satisfied
    ...
    # fb maps to EF+IF; sb maps to ES+IS; nb maps to ENB+PF — use tightest budget (max survival)
