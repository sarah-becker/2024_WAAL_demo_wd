...
**IUU 12-panel: mean λ with final uncertainty plot symbology (geom_contour_filled + heat_grad)**
```{r p_iuu_12panel_mean, fig.width=16, fig.height=12}
# Full 4 × 3 = 12-panel IUU × enforcement heatmap matching p_hm_mean symbology.
# Uses geom_contour_filled + heat_grad palette + theme_bw instead of geom_tile + gradientn.
# Labels shown only in the no-IUU / no-enforcement panel to avoid clutter.
...
p_iuu_12panel_mean <- ggplot(iuu_heatmap_grid, aes(x = coverage, y = efficacy, z = lambda)) +
  ...
  geom_point(data = iuu_fleet_pts %>% filter(efficacy_scenario != "No Mitigation"),
             aes(x = effective_coverage, y = effective_efficacy, shape = compliance_level),
             inherit.aes = FALSE, size = 1.5, color = "black") +
  # Scenario labels (50% compliance points, first panel only)
  geom_label(data = iuu_labels_12,
             aes(x = effective_coverage, y = effective_efficacy, label = efficacy_scenario),
             inherit.aes = FALSE,
             size = 2.5, fill = "white", alpha = 0.8,
             label.padding = unit(0.10, "lines"),
             hjust = 1.05, vjust = 0.5) +
  # No mitigation: diamond in all panels, label in first panel only
...
ggsave(fig_path("28_iuu_12panel_mean"), p_iuu_12panel_mean, width = 16, height = 14, dpi = 200)
