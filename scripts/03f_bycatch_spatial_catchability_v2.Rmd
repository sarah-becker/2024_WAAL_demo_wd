...
```{r load-bird-data}
# Total bird density (all populations combined)
total_bird_density <- rast(here("output/rasters/total_bird_density_all_pops.tif"))

# Bird Island age-class density rasters (from script 02)
# These give the density of BI WAAL per cell for each age class
bi_density_fb <- rast(here("output/rasters/BI_density_fb_final.tif"))
bi_density_sb <- rast(here("output/rasters/BI_density_sb_final.tif"))
bi_density_nb <- rast(here("output/rasters/BI_density_nb_final.tif"))
bi_density_j2j3 <- rast(here("output/rasters/BI_density_j2j3_final.tif"))
bi_density_imm <- rast(here("output/rasters/BI_density_imm_final.tif"))

bi_dists <- list(fb = bi_density_fb, sb = bi_density_sb, nb = bi_density_nb,
                 j2j3 = bi_density_j2j3, imm = bi_density_imm)

# Total BI bird density (sum across age classes)
bi_density_total <- bi_density_fb + bi_density_sb + bi_density_nb +
                    bi_density_j2j3 + bi_density_imm
```
...
For cells with effort but no BPUE study coverage, fill using similarity-based
mean β values. Since β is "pure catchability" (bird density factored out),
averaging β across studies is appropriate.

Gap-fill β = mean of available similarity dimensions (within same fishery type):
- Fleet mean β (if fleet has BPUE data elsewhere)
- Region mean β
- Period mean β
- Fallback: fishery-type mean β
...
# Mean beta by fleet (within fishery type)
beta_by_fleet <- valid_betas %>%
  group_by(fishery_type, fleet) %>%
  summarise(mean_beta = mean(beta, na.rm = TRUE), n = n(), .groups = "drop")

# Mean beta by region (within fishery type)
beta_by_region <- valid_betas %>%
  filter(!is.na(study_region)) %>%
  group_by(fishery_type, study_region) %>%
  summarise(mean_beta = mean(beta, na.rm = TRUE), n = n(), .groups = "drop")

# Mean beta by period (within fishery type)
beta_by_period <- valid_betas %>%
...
cat("Beta summary tables computed:\n")
cat("  By fleet:", nrow(beta_by_fleet), "entries\n")
cat("  By region:", nrow(beta_by_region), "entries\n")
cat("  By period:", nrow(beta_by_period), "entries\n")
cat("  By fishery:", nrow(beta_by_fishery_mean), "entries\n\n")

cat("Mean beta by fishery type:\n")
...
for (i in 1:nrow(fleet_fp_combos)) {
  ...
  # Look up fleet mean beta (if fleet has any BPUE data, same fishery type)
  fleet_beta_val <- beta_by_fleet %>%
    filter(fishery_type == fi, fleet == fl) %>%
    pull(mean_beta)
  has_fleet_beta <- length(fleet_beta_val) > 0 && !is.na(fleet_beta_val[1])

  # Look up period mean beta
  period_beta_val <- beta_by_period %>%
    filter(fishery_type == fi, plot_year_group == yg) %>%
    pull(mean_beta)
  has_period_beta <- length(period_beta_val) > 0 && !is.na(period_beta_val[1])
  ...
  # Assign each gap cell to a region
  ...
  for (reg in gap_regions) {
    ...
    # Look up region mean beta
    region_beta_val <- beta_by_region %>%
      filter(fishery_type == fi, study_region == reg) %>%
      pull(mean_beta)
    has_region_beta <- length(region_beta_val) > 0 && !is.na(region_beta_val[1])

    # Compute fill beta as mean of available similarity dimensions
    fill_components <- c()
    if (has_fleet_beta) fill_components <- c(fill_components, fleet_beta_val[1])
    if (has_region_beta) fill_components <- c(fill_components, region_beta_val[1])
    if (has_period_beta) fill_components <- c(fill_components, period_beta_val[1])

    if (length(fill_components) > 0) {
      fill_beta <- mean(fill_components)
      fill_method <- paste0("similarity_", length(fill_components))
    } else {
      fill_beta <- fishery_beta_val[1]
      fill_method <- "fishery_mean"
    }

    # Assign to all gap cells in this region
    ...
    gap_fill_log <- bind_rows(gap_fill_log, tibble(
      ...
      fill_beta = fill_beta,
      n_components = length(fill_components)
    ))
  }
  ...
  if (nrow(no_region_cells) > 0) {
    fallback_beta <- fishery_beta_val[1]

    catch_rast[no_region_cells$cell] <- fallback_beta
...
write_csv(bycatch_results, here("output/bycatch_by_fleet_period_03f.csv"))

# Save fishery x period summaries
write_csv(byc_by_fp, here("output/bycatch_by_fishery_period_03f.csv"))

# Save per capita rates
write_csv(percap_rates, here("output/bycatch_percapita_03f.csv"))

# Save gap-fill log
write_csv(gap_fill_log, here("output/gap_fill_log_03f.csv"))

# Save coverage info
write_csv(coverage_info, here("output/coverage_info_03f.csv"))
...
write_csv(
  ...
  here("output/bpue_study_betas_03f.csv")
)

# Save fishery-level beta summary
write_csv(beta_by_fishery_type, here("output/beta_by_fishery_type_03f.csv"))
...
for (fi in c("PLL", "DLL")) {
  for (yg in target_year_groups) {
    ...
    writeRaster(mean_catch,
                here(paste0("output/rasters/catchability_", tolower(fi), "_", yg_clean, "_03f.tif")),
                overwrite = TRUE)
  }
}
