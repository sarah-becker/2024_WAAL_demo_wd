...
# Labels for heatmap scenario points at 100% compliance
...
```{r}
# IUU range 0-25% of legal FM, covering scenario values of 0, 3, 7, 20%
iuu_range <- seq(0, 0.25, by = 0.01)
...
iuu_plot_vals <- c(0, 0.03, 0.07, 0.20)
...
iuu_props_plot <- c(0, 0.03, 0.07, 0.20)   # 0% = no IUU (original heatmap)
...
grid_opt2 <- iuu_heatmap_grid %>% filter(iuu_prop %in% c(0,0.03, 0.07, 0.20))
pts_opt2  <- iuu_fleet_pts %>% filter(iuu_prop %in% c(0, 0.03, 0.07, 0.20))
...
p_iuu_12panel_mean <- ggplot(iuu_heatmap_grid, aes(x = coverage, y = efficacy, z = lambda)) +
  ...
  geom_label(data = iuu_labels_12,
             ...
             inherit.aes = FALSE,
             size = 2.0, fill = "white", alpha = 0.8,
             label.padding = unit(0.12, "lines"),
             hjust = -0.05, vjust = 0.5) +
  facet_grid(iuu_label ~ iuu_red_label) +
  scale_shape_manual(values = c("50%" = 0, "75%" = 17, "100%" = 16),
                     name = "Compliance level") +
  scale_fill_manual(values = colorRampPalette(heat_grad)(length(lambda_breaks) - 1),
...
# Use a single IUU proportion (best estimate = 7%)
iuu_best <- 0.07
...
# SD estimated directly from observed annual range (Bird Island 1980–2012):
# SD = (upper - lower) / 3.92, where 3.92 = 2 * 1.96 (range spans ~95% of values).
# A single SD is applied to all stages within each rate type, giving all stages
# the same absolute uncertainty — appropriate since the range comes from pooled data
# that does not distinguish between stages.
sd_juv   <- (0.95 - 0.65) / 3.92  # juvenile survival range 0.65–0.95
sd_adult <- (0.98 - 0.88) / 3.92  # immature/adult survival range 0.88–0.98

surv_ci_from_sd <- function(mean_val, sd) {
  lower <- max(mean_val - 1.96 * sd, 0.01)
  upper <- min(mean_val + 1.96 * sd, 0.9999)
  calc_beta_params(mean_val, lower, upper)
}

surv_params <- list(
  s_Juv = surv_ci_from_sd(s_Juv, sd_juv),   # juvenile
  s_EF  = surv_ci_from_sd(s_EF,  sd_adult), # experienced failed
  s_ENB = surv_ci_from_sd(s_ENB, sd_adult), # experienced non-breeding
  s_ES  = surv_ci_from_sd(s_ES,  sd_adult), # experienced successful
  s_IF  = surv_ci_from_sd(s_IF,  sd_adult), # inexperienced failed
  s_Imm = surv_ci_from_sd(s_Imm, sd_adult), # immature
  s_IS  = surv_ci_from_sd(s_IS,  sd_adult), # inexperienced successful
  s_PF  = surv_ci_from_sd(s_PF,  sd_adult)  # sabbatical
)

# === BREEDING PROBABILITY ===
# Range 0.65–0.75 (Pardo model estimates, Bird Island)
sd_breed <- (0.75 - 0.65) / 3.92
breed_params <- list(
  b_EF  = surv_ci_from_sd(b_EF,  sd_breed),
  b_ENB = surv_ci_from_sd(b_ENB, sd_breed),
  b_ES  = surv_ci_from_sd(b_ES,  sd_breed),
  b_IF  = surv_ci_from_sd(b_IF,  sd_breed),
  b_Imm = surv_ci_from_sd(b_Imm, sd_breed),
  b_IS  = surv_ci_from_sd(b_IS,  sd_breed),
  b_PF  = surv_ci_from_sd(b_PF,  sd_breed)
)

# === RETURN PROBABILITY ===
# Range 0.60–0.70 (Pardo model estimates, Bird Island)
sd_return <- (0.70 - 0.60) / 3.92
return_params <- list(
  r_EF  = surv_ci_from_sd(r_EF,  sd_return),
  r_ENB = surv_ci_from_sd(r_ENB, sd_return),
  r_ES  = surv_ci_from_sd(r_ES,  sd_return),
  r_IF  = surv_ci_from_sd(r_IF,  sd_return),
  r_Imm = surv_ci_from_sd(r_Imm, sd_return),
  r_IS  = surv_ci_from_sd(r_IS,  sd_return),
  r_PF  = surv_ci_from_sd(r_PF,  sd_return)
)

# === BREEDING SUCCESS ===
# Range 0.65–0.80 (Pardo model estimates, Bird Island)
sd_success <- (0.80 - 0.65) / 3.92
success_params <- list(
  k_EF  = surv_ci_from_sd(k_EF,  sd_success),
  k_ENB = surv_ci_from_sd(k_ENB, sd_success),
  k_ES  = surv_ci_from_sd(k_ES,  sd_success),
  k_IF  = surv_ci_from_sd(k_IF,  sd_success),
  k_Imm = surv_ci_from_sd(k_Imm, sd_success),
  k_IS  = surv_ci_from_sd(k_IS,  sd_success),
  k_PF  = surv_ci_from_sd(k_PF,  sd_success)
)
```
...
ggsave(fig_path("28_iuu_12panel_mean"), p_iuu_12panel_mean, width = 16, height = 18, dpi = 200)
