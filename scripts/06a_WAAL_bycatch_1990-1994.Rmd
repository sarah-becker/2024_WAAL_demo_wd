...
**set vital rates (period-specific survival 1990–1994, stage-specific behavioral rates)**
```{r}
# Period-specific pooled survival (Pardo et al. estimates for 1990–1994)
s_Juv   <- 0.78  # juvenile survival
s_Adult <- 0.96  # pooled adult survival (applied uniformly to all adult stages)
...
# === SURVIVAL RATES (period-specific pooled, 1990–1994) ===
# Only survival varies by period; behavioral rate CIs are unchanged from script 05
...
surv_params <- list(
  ...
  s_Adult = calc_beta_params(0.960, 0.920, 0.990)   # pooled adult survival — moderate CI
)
...
# Diagnostic counters — simplified: only s_Juv < s_Adult hierarchy enforced
...
for (k in seq_along(mitigation_scenarios$efficacy)) {
  ...
  for (j in 1:n_sims) {
    ...
    # --- Draw vital rates: pooled survival per period, stage-specific behavioral rates ---
    # Survival: draw s_Adult (pooled) and s_Juv; enforce s_Juv < s_Adult
    ...
    # Assign pooled adult survival to all adult stage sim variables
    sim_s_Imm <- sim_s_Adult; sim_s_EF <- sim_s_Adult; sim_s_ES <- sim_s_Adult
    sim_s_ENB <- sim_s_Adult; sim_s_IF <- sim_s_Adult; sim_s_IS <- sim_s_Adult; sim_s_PF <- sim_s_Adult
...
# Diagnostic counters for fixed-IUU simulation — simplified: only s_Juv < s_Adult
diag2_juv_redraws <- 0
diag2_juv_capped  <- 0
