...
bbox_lookup <- tribble(
  ...
  # --- Huang 2017: Taiwan PLL, SE Atlantic ---
  "25-60 S, 30 W-20 E",
  -60, -25, -30, 20, "",

  # --- Huang 2017: Taiwan PLL, SW Atlantic ---
  "25-60 S, 70-30 W",
  -60, -25, -70, -30, "",

  # --- Huang 2017: Taiwan PLL, SE Pacific (crosses dateline) ---
  "25-60 S, 160-70 W",
  -60, -25, 160, 290, "Crosses dateline; lon_max=290 (=-70+360)",

  # --- Huang 2017: Taiwan PLL, SW Pacific (crosses dateline) ---
  "25-60 S, 130 E-160 W",
  -60, -25, 130, 200, "Crosses dateline; lon_max=200 (=-160+360)",

  # --- Huang 2017: Taiwan PLL, SE Indian ---
  "25-60 S, 75-130 E",
  -60, -25, 75, 130, "",

  # --- Huang 2017: Taiwan PLL, SW Indian ---
  "25-60 S, 20-75 E",
  -60, -25, 20, 75, "",
...
## Flag known issues - zero waal bpue recorded
...
# Assign year groups
year_groups <- c("1990-1994", "1995-1999", "2000-2004", "2005-2009")
...
avail_pll <- pll_data %>% count(fleet, sort = TRUE) 
```
...
avail_dll <- dll_data %>% count(fleet, sort = TRUE) 
```
...
```{r expand-periods}
target_year_groups <- c("1990-1994", "1995-1999", "2000-2004", "2005-2009", "2010-2014", "2015-2019")

# Parse the year_group field, which may contain comma-separated periods.
# Uses overlap logic: any study period overlapping a target period is included.
# Handles shorthand years (e.g. "1990-94"), leading spaces, and arbitrary ranges.
expand_to_target_periods <- function(yg_string) {
  if (is.na(yg_string) | yg_string == "") return(character(0))

  # Target period boundaries (must match target_year_groups order)
  period_bounds <- tibble(
    period = target_year_groups,
    start  = c(1990, 1995, 2000, 2005, 2010, 2015),
    end    = c(1994, 1999, 2004, 2009, 2014, 2019)
  )
  ...
  matched <- map(periods, function(p) {
    # Extract all 2- or 4-digit numbers from the string
    ...
    # Return all target periods that overlap with [start_yr, end_yr]
    period_bounds %>%
...
make_effort_raster <- function(effort_data, fleet_name, yr_group) {
  is_dll <- "dataset" %in% names(effort_data)

  effort_subset <- effort_data %>%
    filter(fleet == fleet_name, year_group == yr_group) %>%
    { if (is_dll) group_by(., Lon, Lat, dataset) else group_by(., Lon, Lat, RFMO) } %>%
    summarise(mean_effort = mean(Hooks, na.rm = TRUE), .groups = "drop")

  if (nrow(effort_subset) < 2) return(NULL)

  # DLL only: check if NZ data is mixed with other sources
  # NZ uses x.0 grid centers; all other DLL sources use x.5 — must rasterize separately
  # to avoid checkerboard artifacts, matching the approach in script 01
  has_nz    <- is_dll && any(effort_subset$dataset == "New Zealand", na.rm = TRUE)
  has_other <- is_dll && any(effort_subset$dataset != "New Zealand", na.rm = TRUE)

  if (has_nz & has_other) {
    # Rasterize non-NZ and NZ separately, then resample NZ to align grids before adding
    non_nz <- effort_subset %>% filter(dataset != "New Zealand") %>%
      select(Lon, Lat, mean_effort)
    nz     <- effort_subset %>% filter(dataset == "New Zealand") %>%
      select(Lon, Lat, mean_effort)

    rast_other <- tryCatch(
      rast(non_nz, type = "xyz", crs = "EPSG:4326"),
      error = function(e) {
        message("Rasterization failed (non-NZ) for fleet: ", fleet_name,
                " period: ", yr_group, " — ", conditionMessage(e))
        NULL
      }
    )
    if (is.null(rast_other)) return(NULL)

    rast_nz <- tryCatch(
      rast(nz, type = "xyz", crs = "EPSG:4326"),
      error = function(e) {
        message("Rasterization failed (NZ) for fleet: ", fleet_name,
                " period: ", yr_group, " — ", conditionMessage(e))
        NULL
      }
    )
    if (is.null(rast_nz)) return(rast_other)

    rast_nz_aligned <- resample(rast_nz, rast_other, method = "bilinear")
    return(subst(subst(rast_other, NA, 0) + subst(rast_nz_aligned, NA, 0), 0, NA))

  } else {
    # Single grid convention — rasterize directly
    tryCatch(
      rast(effort_subset %>% select(Lon, Lat, mean_effort),
           type = "xyz", crs = "EPSG:4326"),
      error = function(e) {
        message("Rasterization failed for fleet: ", fleet_name,
                " period: ", yr_group, " — ", conditionMessage(e))
        NULL
      }
    )
...
create_fleet_map <- function(fishery, fleet_name, effort_data, bpue_data,
  ...
  panels <- map(year_groups, function(yg) {
    ...
    # Fixed Southern Ocean extent for all fleet maps — ensures consistent view
    # and prevents bounding boxes or effort being clipped by dynamic extents
    xlim <- c(-180, 180)
    ylim <- c(-80, 0)
...
# All PLL fleets: union of BPUE and effort fleets covers three cases:
# (1) has BPUE + effort, (2) has effort but no BPUE, (3) has BPUE but no effort
pll_all_fleets <- union(
  bpue_std %>% filter(fishery_type == "PLL") %>% distinct(fleet) %>% pull(fleet),
  pll_data %>% distinct(fleet) %>% pull(fleet)
) %>% sort()

for (fl in pll_all_fleets) {
  p <- create_fleet_map(
    fishery = "PLL",
    fleet_name = fl,
...
# All DLL fleets: union of BPUE and effort fleets
dll_all_fleets <- union(
  bpue_std %>% filter(fishery_type == "DLL") %>% distinct(fleet) %>% pull(fleet),
  dll_data %>% distinct(fleet) %>% pull(fleet)
) %>% sort()

for (fl in dll_all_fleets) {
  p <- create_fleet_map(
    fishery = "DLL",
    fleet_name = fl,
...
pll_gaps <- map_dfr(pll_all_fleets, function(fl) {
...
dll_gaps <- map_dfr(dll_all_fleets, function(fl) {
  map_dfr(target_year_groups, function(yg) {
...
```{r}
# Define SH fleet lists here so this chunk can run independently
pll_effort_fleets_so <- pll_data %>% filter(Lat <= 0) %>% distinct(fleet) %>% pull()
dll_effort_fleets_so <- dll_data %>% filter(Lat <= 0) %>% distinct(fleet) %>% pull()

# Southern Hemisphere only — filter to fleets with effort at lat <= 0
coverage_plot_so <- spatial_gaps %>%
  filter(has_effort) %>%  
  filter((fishery == "PLL" & fleet %in% pll_effort_fleets_so) | (fishery == "DLL" & fleet %in% dll_effort_fleets_so))    

p_heatmap_so <- ggplot(coverage_plot_so,          
   aes(x = year_group, y = fleet, fill = pct_covered)) +       
    geom_tile(color = "white", linewidth = 0.5) +     
  geom_text(aes(label = ifelse(is.na(pct_covered), "no effort",    
                               paste0(pct_covered, "%"))),          
  ...
  scale_fill_gradient2(low = "red", mid = "yellow", high = "darkgreen",   midpoint = 50, na.value = "grey90",     
                       name = "% Effort Cells\nCovered by BPUE" ) +   
  labs(  title = "BPUE Spatial Coverage: Southern Hemisphere Fleets Only",  
         subtitle = "Red = fishing effort exists but no BPUE study covers it. Grey = no effort data for this fleet.",    
         x = "Year Group", y = "Fleet"   ) +     
  theme_minimal() +       
...
ggsave(here("output/maps/bpue_review/coverage_heatmap_southern_hemisphere.png"), p_heatmap_so, width = 14, height = 8, dpi = 300)     
```
...
# Crop to Southern Hemisphere (lat <= 0) — consistent with script 01 map extent
# Shows which effort-only fleets are actually operating in the WAAL range
pll_missing_so <- pll_data %>%
  filter(fleet %in% pll_missing, Lat <= 0) %>%
  group_by(fleet) %>%
  summarise(total_hooks = sum(Hooks, na.rm = TRUE), .groups = "drop") %>%
  arrange(desc(total_hooks))

dll_missing_so <- dll_data %>%
  filter(fleet %in% dll_missing, Lat <= 0) %>%
  group_by(fleet) %>%
  summarise(total_hooks = sum(Hooks, na.rm = TRUE), .groups = "drop") %>%
  arrange(desc(total_hooks))

cat("PLL fleets with effort but no BPUE — Southern Hemisphere hooks:\n")
print(pll_missing_so)
cat("\nDLL fleets with effort but no BPUE — Southern Hemisphere hooks:\n")
print(dll_missing_so)
```
...
# Crop effort fleets to Southern Hemisphere (lat <= 0) before gap summary
# to exclude fleets operating only in northern/tropical regions
pll_effort_fleets_so <- pll_data %>% filter(Lat <= 0) %>% distinct(fleet) %>% pull()
dll_effort_fleets_so <- dll_data %>% filter(Lat <= 0) %>% distinct(fleet) %>% pull()
...
all_fleets <- bind_rows(
  ...
  tibble(fishery = "PLL", fleet = pll_effort_fleets_so),
  tibble(fishery = "DLL", fleet = dll_effort_fleets_so)
) %>%
  distinct()
...
fleet_data_status <- all_fleets %>%
  mutate(
    has_effort = case_when(
      fishery == "PLL" ~ fleet %in% pll_effort_fleets_so,
      fishery == "DLL" ~ fleet %in% dll_effort_fleets_so
    ),
    has_bpue = case_when(
...
## Verify region definitions

```{r map-regions, fig.width=14, fig.height=7}
# Build sf polygons from region_defs for mapping
region_sf <- region_defs %>%
  rowwise() %>%
  mutate(geometry = list(st_polygon(list(matrix(
    c(lon_min, lat_min, lon_max, lat_min, lon_max, lat_max,
      lon_min, lat_max, lon_min, lat_min),
    ncol = 2, byrow = TRUE
  ))))) %>%
  ungroup() %>%
...
ggplot() +
  geom_sf(data = region_sf, aes(fill = region), alpha = 0.3, color = "black", linewidth = 0.5) +
  geom_sf(data = ne_land, fill = "grey80", color = "grey50", linewidth = 0.2) +
  geom_sf_label(data = region_sf, aes(label = region), size = 3) +
  coord_sf(xlim = c(-180, 180), ylim = c(-90, 0), expand = FALSE) +
  scale_fill_brewer(palette = "Set2", guide = "none") +
  labs(title = "Ocean region definitions") +
  theme_minimal()
```
...
## Southern hemisphere gap table (lat < 0 only)

```{r gap-table-sh}
# Rebuild effort summary filtered to southern hemisphere cells only (Lat < 0)
pll_effort_sh <- pll_with_region %>%
  filter(!is.na(region), Lat < 0) %>%
  group_by(fleet, year_group, region, RFMO) %>%
  summarise(total_hooks = sum(Hooks, na.rm = TRUE), .groups = "drop") %>%
  mutate(fishery = "PLL")

dll_effort_sh <- dll_with_region %>%
  filter(!is.na(region), Lat < 0) %>%
  group_by(fleet, year_group, region, RFMO) %>%
  summarise(total_hooks = sum(Hooks, na.rm = TRUE), .groups = "drop") %>%
  mutate(fishery = "DLL")

effort_sh <- bind_rows(pll_effort_sh, dll_effort_sh)

# Gap table: left join effort (SH only) with BPUE where it exists
regional_gap_sh <- effort_sh %>%
  left_join(
    bpue_regional_summary,
    by = c("fishery", "fleet", "year_group", "region")
  ) %>%
...
regional_gap_sh %>% print(n = 200)

gaps_only_sh <- regional_gap_sh %>%
  filter(!has_bpue) %>%
  select(fishery, fleet, RFMO, year_group, region, total_hooks) %>%
  arrange(fishery, fleet, region, year_group)

gaps_only_sh %>% print(n = 200)

write_csv(regional_gap_sh,  here("output/regional_bpue_gap_table_SH.csv"))
write_csv(gaps_only_sh,     here("output/regional_bpue_gaps_only_SH.csv"))
```
...
bpue_summary_full <- bpue_by_region %>%
  group_by(fishery_type, superfleet, fleet, plot_year_group, bpue_region) %>%
  summarise(
    n_studies = n_distinct(citation),
    n_bpue = n_distinct(row_id),
  ...
  arrange(fishery_type, superfleet, fleet, plot_year_group, bpue_region)
...
bpue_summary_fleet <- bpue_by_region %>%
  group_by(fishery_type, superfleet, fleet) %>%
  summarise(
    n_studies = n_distinct(citation),
    n_bpue = n_distinct(row_id),
    mean_bpue = round(mean(bpue[!duplicated(row_id)], na.rm = TRUE), 4),
  ...
  arrange(fishery_type, superfleet, fleet)
...
# Methods reporting summary

```{r methods-reporting-summary}
# All counts use bpue_raw (one row per study x fleet x period entry in the CSV)
# Only rows with actual BPUE values (not temporal gap placeholders) are counted

bpue_data <- bpue_raw %>% filter(!is.na(bpue))

# --- Overall counts ---
n_studies_total  <- n_distinct(bpue_data$citation)
n_bpue_total     <- nrow(bpue_data)
n_dll            <- bpue_data %>% filter(fishery_type == "DLL") %>% nrow()
n_pll            <- bpue_data %>% filter(fishery_type == "PLL") %>% nrow()
n_studies_dll    <- bpue_data %>% filter(fishery_type == "DLL") %>% distinct(citation) %>% nrow()
n_studies_pll    <- bpue_data %>% filter(fishery_type == "PLL") %>% distinct(citation) %>% nrow()

cat("Total unique studies assessed:", n_studies_total, "\n")
cat("Total BPUE values used:       ", n_bpue_total, "\n")
cat("  DLL studies:", n_studies_dll, "| BPUE values:", n_dll, "\n")
cat("  PLL studies:", n_studies_pll, "| BPUE values:", n_pll, "\n\n")

# --- Direct vs downscaled vs mean ---
bpue_source_summary <- bpue_data %>%
  mutate(source_clean = case_when(
    str_detect(tolower(bpue_source), "direct") ~ "direct",
    str_detect(tolower(bpue_source), "downscal") ~ "downscaled",
    str_detect(tolower(bpue_source), "mean") ~ "mean",
    TRUE ~ "other/unspecified"
  )) %>%
  count(fishery_type, source_clean, name = "n_bpue") %>%
  arrange(fishery_type, source_clean)

cat("BPUE source breakdown:\n")
print(bpue_source_summary)

# --- Geographic coverage: unique regions with BPUE data ---
region_coverage <- bpue_by_region %>%
  distinct(fishery_type, bpue_region) %>%
  arrange(fishery_type, bpue_region)

cat("\nRegions with BPUE coverage:\n")
print(region_coverage)

# --- Temporal coverage: periods with at least one BPUE value ---
period_coverage <- bpue_data %>%
  count(fishery_type, year_group, name = "n_bpue") %>%
  arrange(fishery_type, year_group)

cat("\nTemporal coverage (BPUE values per period):\n")
print(period_coverage)

# --- Fleet coverage: how many fleets have BPUE data by fishery ---
fleet_coverage <- bpue_data %>%
  group_by(fishery_type) %>%
  summarise(n_fleets = n_distinct(fleet),
            fleets = paste(sort(unique(fleet)), collapse = ", "),
            .groups = "drop")

cat("\nFleet coverage:\n")
print(fleet_coverage)
```
