...
cat("Grid alignment check — first 5 unique Lon values per source (before wrapping):\n")
cat("  CCAMLR:     ", paste(head(sort(unique(ccamlr_std$Lon)), 5), collapse = ", "), "\n")
cat("  Argentina:  ", paste(head(sort(unique(arg_std$Lon)), 5), collapse = ", "), "\n")
cat("  Chile:      ", paste(head(sort(unique(chile_std$Lon)), 5), collapse = ", "), "\n")
cat("  Falklands:  ", paste(head(sort(unique(falk_std$Lon)), 5), collapse = ", "), "\n")
cat("  Namibia:    ", paste(head(sort(unique(namibia_std$Lon)), 5), collapse = ", "), "\n")
cat("  S. Africa:  ", paste(head(sort(unique(saf_std$Lon)), 5), collapse = ", "), "\n")
cat("  New Zealand:", paste(head(sort(unique(nz_std$Lon)), 5), collapse = ", "), "\n")
...
dll_summary <- dll_data %>%
  ...
  summarise(
    ...
    min_lat  = min(.data[["Lat"]],  na.rm = TRUE),
    max_lat  = max(.data[["Lat"]],  na.rm = TRUE),
    min_lon  = min(.data[["Lon"]],  na.rm = TRUE),
    max_lon  = max(.data[["Lon"]],  na.rm = TRUE),
    total_hooks = sum(.data[["Hooks"]], na.rm = TRUE),
    n_records   = n(),
...
calculate_overlap <- function(bird_layers, fishing_rast, extent) {
  ...
  overlaps <- map(bird_layers, function(bird_rast) {
    ...
    # Normalize so values represent the proportion of the population in each cell
    bird_norm <- bird_crop / global(bird_crop, fun = "sum", na.rm = TRUE)[1,1]
    # Resample normalized distribution to match fishing raster grid
    # method = "average" aggregates all fine-resolution bird cells within each
    # coarse 5-degree fishing cell (na.rm implicit), which correctly captures
    # high at-sea density around land masses like South Georgia rather than
    # point-sampling at the cell center where land NA values can dominate
    bird_resample <- resample(bird_norm, fishing_crop, method = "average")
    # Overlap index: hooks weighted by proportion of population present
    overlap <- fishing_crop * bird_resample
    return(overlap)
  })
