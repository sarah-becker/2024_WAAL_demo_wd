...
```{r load-data}
# Current 04_v3 output date (includes J1)
date_sfx_04v3 <- "2026-03-29"

# Bycatch by fishery and period (04_v3 — mean annual BI bycatch, all ages summed, includes J1)
byc_period <- read_csv(here(paste0("output/bycatch_by_fishery_period_04_v3_", date_sfx_04v3, ".csv")),
                       show_col_types = FALSE)

# Per-capita fishing mortality by period and age class (04_v3, includes J1)
percap <- read_csv(here(paste0("output/percap_rates_by_period_04_v3_", date_sfx_04v3, ".csv")),
                   show_col_types = FALSE)
```
...
```{r effort-data}
# Total hooks by fishery type and period — derived from raw effort files
# (no hooks_by_fleet_period file exists for 04_v3; same source data as script 01)
# Step 1: sum hooks across months within each year per cell; Step 2: sum across cells
# per year; Step 3: average across years per period
pll_hooks <- read_csv(here("output/pll_effort_by_fleet.csv"), show_col_types = FALSE) %>%
  ...
  group_by(year_group) %>%
  summarise(mean_annual_hooks = mean(annual_hooks, na.rm = TRUE),
            .groups = "drop") %>%
  mutate(fishery = "PLL")

dll_hooks <- read_csv(here("output/dll_effort_by_fleet.csv"), show_col_types = FALSE) %>%
  filter(Year >= 1990, Year <= 2009) %>%
  mutate(year_group = case_when(
    Year >= 1990 & Year <= 1994 ~ "1990-1994",
    Year >= 1995 & Year <= 1999 ~ "1995-1999",
...
hooks_by_period <- bind_rows(pll_hooks, dll_hooks) %>%
  mutate(
...
catchability_by_period <- byc_period %>%
  left_join(hooks_by_period, by = c("year_group", "fishery")) %>%
  mutate(
    period  = factor(year_group, levels = model_periods),
    fishery = factor(fishery, levels = c("PLL", "DLL")),
...
# Mean annual hooks per fleet × period × fishery — derived from raw effort files
pll_fleet_period <- read_csv(here("output/pll_effort_by_fleet.csv"), show_col_types = FALSE) %>%
  filter(Year >= 1990, Year <= 2009) %>%
  mutate(fleet = ifelse(is.na(Flag), "Other", Flag),
         fishery = "PLL",
         year_group = case_when(
           Year >= 1990 & Year <= 1994 ~ "1990-1994",
           Year >= 1995 & Year <= 1999 ~ "1995-1999",
  ...
  group_by(fishery, fleet, year_group, Year) %>%
  summarise(annual_hooks = sum(Hooks, na.rm = TRUE), .groups = "drop") %>%
  group_by(fishery, fleet, year_group) %>%
  summarise(mean_annual_hooks = mean(annual_hooks, na.rm = TRUE), .groups = "drop")

dll_fleet_period <- read_csv(here("output/dll_effort_by_fleet.csv"), show_col_types = FALSE) %>%
  filter(Year >= 1990, Year <= 2009) %>%
  rename(fleet = Fleet) %>%
  mutate(fishery = "DLL",
         year_group = case_when(
...
fleet_effort <- bind_rows(pll_fleet_period, dll_fleet_period) %>%
  mutate(
    period  = factor(year_group, levels = model_periods),
    fishery = factor(fishery, levels = c("PLL", "DLL"))
  )
...
```{r decomp-load}
# 04_v3 counterfactual files — generated by Section 12b of script 04, includes J1
# Uses same period labels as 04_v3 ("1990-1994" etc.)
date_sfx_04v3_decomp <- "2026-03-29"

# Observed bycatch from 04_v3
byc_obs_03g <- read_csv(
  here(paste0("output/bycatch_by_fishery_period_04_v3_", date_sfx_04v3_decomp, ".csv")),
  show_col_types = FALSE
)

# Counterfactual: baseline beta × current effort (effort change only)
byc_eff_only <- read_csv(
  here(paste0("output/bycatch_effort_only_04_v3_", date_sfx_04v3_decomp, ".csv")),
  show_col_types = FALSE
)

# Counterfactual: current beta × baseline effort (catchability change only)
byc_catch_only <- read_csv(
  here(paste0("output/bycatch_catchability_only_04_v3_", date_sfx_04v3_decomp, ".csv")),
  show_col_types = FALSE
)
...
# 04_v3 uses same period labels as Clay ("1990-1994" etc.) — no remapping needed
# age_cols_decomp includes j1 now that 04_v3 outputs include it
age_cols_decomp <- c("fb", "sb", "nb", "j1", "j2j3", "imm")
...
pop_by_effort_period <- dem_props_07 %>%
  ...
  rename(year_group = Period)

# Baseline (1990-1994) population vector — fixed denominator for all periods
...
```{r fm-decomposition}
baseline_03g <- "1990-1994"
later_03g    <- c("1995-1999", "2000-2004", "2005-2009")
...
p4 <- ggplot(decomp_bars, aes(x = year_group, y = pct_reduction, fill = component)) +
  ...
  labs(
    x        = "Period",
    y        = "% reduction in FM relative to 1990\u20131994 baseline",
