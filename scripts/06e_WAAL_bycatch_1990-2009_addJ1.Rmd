...
mitigation_scenarios <- tibble(
  ...
  efficacy = c(eff_hsd_acap,                                            # Bell et al. 2025 ACAP: HSD
  ...
  source = c(rep("Bell ACAP", 6), "Theoretical")
)

mitigation_scenarios %>% mutate(efficacy_pct = round(efficacy * 100, 1)) %>% print()

# Named color vector — HSD in orange, 2/3 combos light-to-medium green, mean 2/3 and 3/3 darker green, elimination near-black
scenario_colors <- c(
  "HSD"                = "#d95f02",
  "BLW + NS"           = "#a1d99b",
  "BLW + BSL"          = "#74c476",
...
# 1990-2009 full period: baseline with no mitigation applied
...
efficacy_scenarios <- list(
  # Baseline
  "no_mitigation"    = fleet_mitigation %>% mutate(base_efficacy = 0),
  # ACAP 2023 spec
...
scenario_plot <- scenario_results %>%
  ...
  mutate(
    efficacy_label = case_when(
      efficacy_scenario == "no_mitigation" ~ "No mitigation",
...
key_scenarios <- c("no_mitigation__compliance_100",
...
no_mit_lambda <- scenario_results %>%
  filter(scenario == "no_mitigation__compliance_100") %>% pull(lambda)
...
hist_weighted_coverage <- if (nrow(has_mit_hist) > 0) {
  sum(has_mit_hist$total_byc * has_mit_hist$compliance) / sum(hist_fleet_cov$total_byc)
} else { 0 }
hist_weighted_efficacy <- if (nrow(has_mit_hist) > 0) {
  sum(has_mit_hist$total_byc * has_mit_hist$base_efficacy) / sum(has_mit_hist$total_byc)
} else { 0 }
...
reference_pts <- tibble(
  ref_label          = c("No mitigation", paste0("Historical (", target_period_byc, ")")),
  effective_coverage = c(0, hist_weighted_coverage),
  effective_efficacy = c(0, hist_weighted_efficacy),
  lambda             = c(no_mit_lambda, hist_lambda),
  period             = target_period_byc
)
...
p_heatmap <- ggplot(heatmap_data, aes(x = coverage, y = efficacy, z = lambda)) +
  ...
  geom_label(data = heatmap_labels,
             ...
             hjust = 1.1, vjust = 0.5) +
  # Reference point: no mitigation (black circle, same size as compliance symbols)
  geom_point(data = reference_pts %>% filter(ref_label == "No mitigation"),
             aes(x = effective_coverage, y = effective_efficacy),
             inherit.aes = FALSE,
             size = 2, shape = 16, color = "black") +
  geom_label(data = reference_pts %>% filter(ref_label == "No mitigation"),
...
for (i in 1:nrow(iuu_scenario_output)) {
  ...
  ns_Juv1 <- 1 - nm_j1_ -
    (dem_fm_j1 + pel_fm_j1) * (1 - mit_eff * red) -
    iuu_j1 * (1 - iuu_red)
  ns_Juv <- 1 - nm_j -
...
calc_lambda_iuu_blanket <- function(mit_eff, cov, iuu_p, iuu_red) {
  ...
  ns_Juv1 <- 1 - nm_j1_ - (dem_fm_j1  + pel_fm_j1)  * (1 - mit_eff * cov) - iuu_j1  * (1 - iuu_red)
...
p_iuu_12panel_mean <- ggplot(iuu_heatmap_grid, aes(x = coverage, y = efficacy, z = lambda)) +
  ...
  geom_point(data = iuu_fleet_pts %>% filter(efficacy_scenario != "No Mitigation"),
  ...
  # No mitigation: diamond in all panels, label in top-right panel only
  geom_point(data = iuu_fleet_pts %>%
               filter(efficacy_scenario == "No Mitigation", compliance_level == "100%"),
             aes(x = effective_coverage, y = effective_efficacy),
             inherit.aes = FALSE, size = 2.5, shape = 18, color = "black") +
  geom_label(data = iuu_fleet_pts %>%
             ...
             aes(x = effective_coverage, y = effective_efficacy + 0.04,
                 label = "No mitigation"),
             inherit.aes = FALSE,
             size = 2.5, fill = "white", alpha = 0.85, color = "black",
...
for (k in seq_along(mitigation_scenarios$efficacy)) {
  ...
  for (j in 1:n_sims) {
    ...
    for (i in seq_along(coverage_seq)) {
      ...
      ns_Juv1 <- 1 - sim_nm_j1 -
        (sim_dem_fm_j1 + sim_pel_fm_j1) * (1 - mit_eff * red) -
        sim_iuu_j1
      ns_Juv <- 1 - sim_nm_juv -
...
# Probability surface: P(lambda >= 1) at each scenario × coverage, mapped to efficacy on y-axis
...
p_hm_mean <- ggplot(heatmap_data, aes(x = coverage, y = efficacy, z = lambda)) +
  geom_contour_filled(breaks = lambda_breaks) +
  geom_contour(aes(z = lambda), breaks = 1, color = "white", linewidth = 1.2) +
  geom_line(data = scenario_heatmap_pts,
            aes(x = effective_coverage, y = effective_efficacy, group = efficacy_scenario),
...
# Interpolate P(lambda >= 1) to a full efficacy × coverage grid so the heatmap
# is not blank below the lowest Bell scenario efficacy.
# Anchor: at efficacy = 0, P(lambda >= 1) = 0 for every coverage level.
...
p_hm_prob <- ggplot(prob_heatmap_full,
