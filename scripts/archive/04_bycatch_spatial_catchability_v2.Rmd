...
also, may want to update the gap filling fallback calculation (mean per fishery) to be hooks-weighted
...
birds of that age class. J1 is excluded as it is not a stage in the demographic model.
...
# (less than prop_BI because J1 is excluded — not a stage in the demographic model)
...
assign_best_period <- function(yg_string) {
  ...
  matched <- map(periods, function(p) {
    ...
    if (p == "1995-2000") return("1995-1999")
    if (p == "2005-2010") return("2005-2009")
    if (p == " 1996-2000") return("1995-1999")
...
cat("\n=== CUMULATIVE BI BYCATCH OVER FULL STUDY PERIOD (1990-2009) ===\n")
...
write_csv(percap_by_period_table,
          here(paste0("output/percap_rates_by_period_03f_", date_sfx, ".csv")))
```
...
ggsave(here(paste0("output/maps/percap_by_period_03f_", date_sfx, ".png")),
...
write_csv(
  ...
  here(paste0("output/percap_rates_by_year_03f_", date_sfx, ".csv"))
)
```
...
print(p_percap_year)
ggsave(here(paste0("output/maps/percap_by_year_03f_", date_sfx, ".png")),
...
for (fi in c("PLL", "DLL")) {
  ...
  print(combined)
  ggsave(here(paste0("output/maps/catchability_", tolower(fi), "_03f_", date_sfx, ".png")),
...
for (fi in c("PLL", "DLL")) {
  ...
  print(combined)
  ggsave(here(paste0("output/maps/bycatch_density_", tolower(fi), "_03f_", date_sfx, ".png")),
         plot = combined, width = 14, height = 12, dpi = 300)
}
```
...
for (fi in c("PLL", "DLL")) {
  ...
  print(combined_age)
  ggsave(here(paste0("output/maps/bycatch_by_age_", tolower(fi), "_03f_", date_sfx, ".png")),
...
ggsave(here(paste0("output/maps/bycatch_by_age_combined_03f_", date_sfx, ".png")),
