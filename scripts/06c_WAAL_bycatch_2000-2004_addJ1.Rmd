...
in_sfx  <- "2026-03-24"  # <-- update when you re-run script 04
...
# Used consistently across all period scripts (06a–d)
...
# Individual scenario efficacy scalars — used in mitigation_scenarios below
...
**historical fleet mitigation — 2000-2004 baseline (placeholders)**
```{r hist-fleet-mitigation}
# Historical mitigation status for 2000-2004 — early CCAMLR mitigation adoption
# PLACEHOLDERS: CCAMLR and Falklands beginning partial adoption; update from literature
hist_fleet_mitigation <- tribble(
  ~fishery, ~fleet,                           ~status,    ~compliance,  ~notes,
  "DLL",    "CCAMLR",                         "partial",  0.30,         "early CCAMLR measures, limited compliance",
  "DLL",    "Argentina",                      "none",     0.0,          "no mitigation",
  "DLL",    "Chile",                          "none",     0.0,          "no mitigation",
  "DLL",    "Falklands",                      "partial",  0.20,         "early partial adoption",
...
# Reference points: no-mitigation, historical (period)
...
p_heatmap <- ggplot(heatmap_data, aes(x = coverage, y = efficacy, fill = lambda)) +
  ...
  geom_label(data = heatmap_labels,
             ...
             hjust = -0.1, vjust = 0.5) +
  # Reference points: no-mitigation, historical (gold diamonds)
...
calc_lambda_iuu_blanket <- function(mit_eff, cov, iuu_p, iuu_red) {
  ...
  ns_Juv1<- 1 - nm_j1i- (dem_fm_j1  + pel_fm_j1)  * (1 - mit_eff * cov) - iuu_j1  * (1 - iuu_red)
...
calc_lambda_fleet_iuu <- function(fleet_percap_df, scenario_df, iuu_p, iuu_red) {
  ...
  ns_Juv1<- 1 - nm_j1i- s_dem_j1  - s_pel_j1  - iuu_j1  * (1 - iuu_red)
...
#          heterogeneity. Wider and more conservative.
...
# Diagnostic counters for redraws (tracked across all scenarios, reported once)
diag_juv_redraws <- 0     # times s_Juv was redrawn to satisfy s_Juv < s_Adult
...
for (k in seq_along(mitigation_scenarios$efficacy)) {
  ...
  for (j in 1:n_sims) {
    ...
    # --- Draw all vital rates freely from their distributions ---
...
# Diagnostic counters for fixed-IUU simulation
diag2_juv_redraws <- 0
diag2_juv_capped <- 0
