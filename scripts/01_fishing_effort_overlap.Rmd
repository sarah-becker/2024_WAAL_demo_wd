...
# Create scaled mean annual effort (in units of 10^3 hooks)
rast_dll_mean_scaled <- rast_dll / 1e3
...
# Create scaled cumulative effort (in units of 10^3 hooks)
rast_dll_cumulative_scaled <- rast_dll_cumulative / 1e3
```
...
map_fishing_effort <- function(rast_obj, title, extent, transform = "sqrt",
  ...
  if (discrete_bins) {
    # Define original bin values (in units of 10^3 or 10^6 hooks)
    original_breaks <- c(0, 5, 10, 25, 50, 100, 500, Inf)
    labels <- c("0", "5", "10", "25", "50", "100", "500", ">500")
    ...
    p <- p + scale_fill_stepsn(
      ...
      guide = guide_colorsteps(even.steps = FALSE, show.limits = TRUE)
    )
  } else {
    # Continuous gradient
...
p_dem_mean_scaled_sqrt <- map_fishing_effort(
  rast_dll_mean_scaled,
  "Mean Annual Demersal Longline Effort (1990-2009)\nScaled: 10^3 hooks (sqrt transformed)",
  ext_dem,
  transform = "sqrt",
  discrete_bins = TRUE
)
...
p_dem_mean_scaled_log <- map_fishing_effort(
  rast_dll_mean_scaled,
  "Mean Annual Demersal Longline Effort (1990-2009)\nScaled: 10^3 hooks (log transformed)",
  ext_dem,
  transform = "log",
  discrete_bins = TRUE
)
...
# Save scaled mean effort rasters
...
# Save scaled cumulative effort rasters
