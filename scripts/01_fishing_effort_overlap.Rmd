...
# Aggregate to mean annual effort by location
pll_annual <- pll_data %>%
  group_by(Lon, Lat, Year) %>%
  summarise(mean_effort = mean(Hooks, na.rm = TRUE), .groups = 'drop')

# Convert to spatial points
pll_points <- vect(pll_annual, geom = c("Lon", "Lat"), crs = "EPSG:4326")

# Create 5-degree resolution raster
pll_extent <- ext(
  min(pll_annual$Lon) - 2.5,
  max(pll_annual$Lon) + 2.5,
  min(pll_annual$Lat) - 2.5,
  max(pll_annual$Lat) + 2.5
)

rast_pll <- rast(
  xmin = pll_extent[1], xmax = pll_extent[2],
  ymin = pll_extent[3], ymax = pll_extent[4],
  resolution = 5,
  crs = "EPSG:4326"
)

# Rasterize mean annual effort
...
# Convert to spatial points
dll_points <- vect(dll_annual, geom = c("Lon", "Lat"), crs = "EPSG:4326")

# Create 5-degree resolution raster
dll_extent <- ext(
  min(dll_annual$Lon) - 2.5,
  max(dll_annual$Lon) + 2.5,
  min(dll_annual$Lat) - 2.5,
  max(dll_annual$Lat) + 2.5
)

rast_dll <- rast(
  xmin = dll_extent[1], xmax = dll_extent[2],
  ymin = dll_extent[3], ymax = dll_extent[4],
  resolution = 5,
  crs = "EPSG:4326"
)

# Rasterize mean annual effort
rast_dll <- rasterize(dll_points, rast_dll, field = "mean_effort", fun = mean)
```
...
ext_pel <- c(-180, 180, -50, 0)   # Pelagic extent
...
map_fishing_effort <- function(rast_obj, title, extent, transform = "sqrt") {
  ...
  ggplot(df, aes(x = x, y = y, fill = effort_trans)) +
    geom_raster() +
