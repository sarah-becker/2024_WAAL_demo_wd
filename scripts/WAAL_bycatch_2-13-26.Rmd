...
coverage_info <- read_csv(here("output/coverage_info_03f.csv"), show_col_types = FALSE)

# For baseline period (1990-1995), classify each fleet's BPUE source
...
ggplot(fleet_byc_scenarios, aes(x = scenario_label, y = byc_mitigated, fill = fleet_label)) +
  geom_col() +
...
key_scenarios <- c("0_no_mitigation", "1_current", "3_all_pll_3of3",
                   "4_full_compliance", "5_target_top3_pll")
...
ggplot(cumul_results, aes(x = cumul_byc_pct, y = lambda,
                          group = scenario, color = scenario)) +
...
ggplot(waterfall_data %>% filter(step > 0), aes(x = fleet_label, y = lambda_gain)) +
...
ggplot(heatmap_data, aes(x = coverage, y = efficacy, fill = lambda)) +
  ...
  labs(x = "% bycatch-weighted coverage (fisheries with mitigation)",
       ...
       subtitle = "White contour = \u03BB = 1 (stability); diamonds = fleet management scenarios") +
  coord_cartesian(expand = FALSE) +
  theme_bw() +
...
ggplot(heatmap_data, aes(x = coverage, y = efficacy, fill = lambda >= 1)) +
  geom_tile() +
  # Overlay fleet scenario points
  geom_point(data = scenario_heatmap_pts,
             aes(x = effective_coverage, y = effective_efficacy),
             inherit.aes = FALSE,
             size = 3.5, shape = 18, color = "white") +
  geom_point(data = scenario_heatmap_pts,
             aes(x = effective_coverage, y = effective_efficacy),
             inherit.aes = FALSE,
             size = 3, shape = 18, color = "black") +
  ...
  labs(x = "% bycatch-weighted coverage (fisheries with mitigation)",
       ...
       subtitle = "Diamonds = fleet-level management scenarios") +
  coord_cartesian(expand = FALSE) +
  theme_bw() +
  theme(panel.grid = element_blank())
```
