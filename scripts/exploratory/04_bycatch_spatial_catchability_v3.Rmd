---
title: "Wandering Albatross Bycatch - Spatial Catchability v3 (Fleet-by-Period, Counterfactuals)"
...
# Section 9b: Counterfactual Bycatch for FM Decomposition

Compute two counterfactual bycatch estimates for each later period relative to the
1990-1995 baseline, to decompose the observed FM change into effort-driven and
catchability-driven components.

- **(a) effort_only**: use 1990-1995 beta rasters × current-period effort rasters.
  Holds catchability fixed at baseline; change reflects effort reduction only.
- **(b) catchability_only**: use current-period beta rasters × 1990-1995 effort rasters.
  Holds effort fixed at baseline; change reflects catchability (beta) reduction only.

Only fleet × fishery combinations that have BOTH baseline AND current-period rasters
are included, ensuring a fair comparison.

```{r calculate-bycatch-counterfactuals}
baseline_period <- "1990-1995"
later_periods   <- c("1996-2000", "2001-2005", "2006-2010")

bycatch_effort_only       <- tibble()
bycatch_catchability_only <- tibble()

for (yg in later_periods) {
  combos_yg <- fleet_fp_combos %>% filter(year_group == yg)

  for (i in 1:nrow(combos_yg)) {
    fi <- combos_yg$fishery[i]
    fl <- combos_yg$fleet[i]

    key_current  <- paste(fi, fl, yg,              sep = "|")
    key_baseline <- paste(fi, fl, baseline_period, sep = "|")

    effort_current  <- effort_rasters[[key_current]]
    effort_baseline <- effort_rasters[[key_baseline]]
    catch_current   <- catchability_rasters[[key_current]]
    catch_baseline  <- catchability_rasters[[key_baseline]]

    # Skip if either current OR baseline rasters are missing
    if (is.null(effort_current)  || is.null(catch_current))  next
    if (is.null(effort_baseline) || is.null(catch_baseline)) next
    if (is.logical(catch_current)  && is.na(catch_current))  next
    if (is.logical(catch_baseline) && is.na(catch_baseline)) next

    # (a) Effort-only: baseline beta × current effort
    byc_eff_only <- map_dbl(names(bi_dists_aligned), function(age) {
      global(catch_baseline * effort_current * bi_dists_aligned[[age]], "sum", na.rm = TRUE)[1, 1]
    })
    names(byc_eff_only) <- names(bi_dists_aligned)

    bycatch_effort_only <- bind_rows(bycatch_effort_only, tibble(
      fishery  = fi, fleet = fl, year_group = yg,
      bi_total = sum(byc_eff_only),
      fb       = byc_eff_only["fb"],
      sb       = byc_eff_only["sb"],
      nb       = byc_eff_only["nb"],
      j2j3     = byc_eff_only["j2j3"],
      imm      = byc_eff_only["imm"]
    ))

    # (b) Catchability-only: current beta × baseline effort
    byc_catch_only <- map_dbl(names(bi_dists_aligned), function(age) {
      global(catch_current * effort_baseline * bi_dists_aligned[[age]], "sum", na.rm = TRUE)[1, 1]
    })
    names(byc_catch_only) <- names(bi_dists_aligned)

    bycatch_catchability_only <- bind_rows(bycatch_catchability_only, tibble(
      fishery  = fi, fleet = fl, year_group = yg,
      bi_total = sum(byc_catch_only),
      fb       = byc_catch_only["fb"],
      sb       = byc_catch_only["sb"],
      nb       = byc_catch_only["nb"],
      j2j3     = byc_catch_only["j2j3"],
      imm      = byc_catch_only["imm"]
    ))
  }
}

cat("\n=== COUNTERFACTUAL BYCATCH: EFFORT-ONLY ===\n")
bycatch_effort_only %>%
  group_by(fishery, year_group) %>%
  summarise(across(c(bi_total, fb, sb, nb, j2j3, imm), sum), .groups = "drop") %>%
  mutate(across(where(is.numeric), ~round(.x, 4))) %>%
  print(n = 30)

cat("\n=== COUNTERFACTUAL BYCATCH: CATCHABILITY-ONLY ===\n")
bycatch_catchability_only %>%
  group_by(fishery, year_group) %>%
  summarise(across(c(bi_total, fb, sb, nb, j2j3, imm), sum), .groups = "drop") %>%
  mutate(across(where(is.numeric), ~round(.x, 4))) %>%
  print(n = 30)
...
cat("\n=== PER CAPITA BYCATCH RATES BY PERIOD (PLL + DLL combined) ===\n")
cat("Mean annual per capita fishing mortality for each age class × period\n\n")
...
write_csv(percap_by_period_table,
          here(paste0("output/percap_rates_by_period_03g_", date_sfx, ".csv")))
```

```{r percap-by-period-plot, fig.width = 10, fig.height = 6}
# Ordered age-class labels for legend
age_order  <- c("j2j3", "imm", "fb", "sb", "nb")
age_labels <- c(
...
ggsave(here(paste0("output/maps/percap_by_period_03g_", date_sfx, ".png")),
...
for (yr in all_years) {
  ...
  for (fi in c("PLL", "DLL")) {
    ...
    for (fl in fleets_in_year) {
      ...
      byc_by_age <- map_dbl(names(bi_dists_aligned), function(age) {
        global(catch_r * effort_r * bi_dists_aligned[[age]], "sum", na.rm = TRUE)[1, 1]
      })
      names(byc_by_age) <- names(bi_dists_aligned)
...
write_csv(
  ...
  here(paste0("output/percap_rates_by_year_03g_", date_sfx, ".csv"))
)
```
...
print(p_percap_year)
ggsave(here(paste0("output/maps/percap_by_year_03g_", date_sfx, ".png")),
...
for (fi in c("PLL", "DLL")) {
  ...
  print(combined)
  ggsave(here(paste0("output/maps/catchability_", tolower(fi), "_03g_", date_sfx, ".png")),
...
for (fi in c("PLL", "DLL")) {
  ...
  print(combined)
  ggsave(here(paste0("output/maps/bycatch_density_", tolower(fi), "_03g_", date_sfx, ".png")),
         plot = combined, width = 14, height = 12, dpi = 300)
}
```
...
age_order_map  <- c("j2j3", "imm", "fb", "sb", "nb")
age_labels_map <- c(
  j2j3 = "Juveniles (J2-J3)",
  imm  = "Immatures",
  fb   = "Failed breeders",
...
for (fi in c("PLL", "DLL")) {
  ...
  for (age in age_order_map) {
    ...
    for (yg in target_year_groups) {
      ...
      for (k in matching_keys) {
        ...
        er <- effort_rasters[[k]]
        if (!is.logical(cr)) {
          cell_byc <- cr * er * bi_dists_aligned[[age]]
          cell_byc[is.na(cell_byc)] <- 0
  ...
  print(combined_age)
  ggsave(here(paste0("output/maps/bycatch_by_age_", tolower(fi), "_03g_", date_sfx, ".png")),
...
ggsave(here(paste0("output/maps/bycatch_by_age_combined_03g_", date_sfx, ".png")),
...
write_csv(bycatch_results, here(paste0("output/bycatch_by_fleet_period_03g_", date_sfx, ".csv")))

# Save fishery x period summaries
write_csv(byc_by_fp, here(paste0("output/bycatch_by_fishery_period_03g_", date_sfx, ".csv")))

# Save per capita rates
write_csv(percap_rates, here(paste0("output/bycatch_percapita_03g_", date_sfx, ".csv")))

# Save gap-fill log
write_csv(gap_fill_log, here(paste0("output/gap_fill_log_03g_", date_sfx, ".csv")))

# Save coverage info
write_csv(coverage_info, here(paste0("output/coverage_info_03g_", date_sfx, ".csv")))
...
write_csv(
  ...
  here(paste0("output/bpue_study_betas_03g_", date_sfx, ".csv"))
)

# Save fishery-level beta summary
write_csv(beta_by_fishery_type, here(paste0("output/beta_by_fishery_type_03g_", date_sfx, ".csv")))
...
for (fi in c("PLL", "DLL")) {
  for (yg in target_year_groups) {
    ...
    writeRaster(mean_catch,
                here(paste0("output/rasters/catchability_", tolower(fi), "_", yg_clean, "_03g_", date_sfx, ".tif")),
                overwrite = TRUE)
  }
}
...
## Decomposition tables (new in 03g)

```{r export-decomposition-tables}
# Scalar beta summary per fleet × period (from Section 8 gap-fill setup)
write_csv(beta_by_fleet_period,
          here(paste0("output/beta_by_fleet_period_03g_", date_sfx, ".csv")))

# Counterfactual bycatch: effort-only (baseline beta × current effort), summed by fishery × period
write_csv(
  bycatch_effort_only %>%
    group_by(fishery, year_group) %>%
    summarise(across(c(bi_total, fb, sb, nb, j2j3, imm), sum), .groups = "drop"),
...
# Counterfactual bycatch: catchability-only (current beta × baseline effort), summed by fishery × period
write_csv(
  bycatch_catchability_only %>%
    group_by(fishery, year_group) %>%
    summarise(across(c(bi_total, fb, sb, nb, j2j3, imm), sum), .groups = "drop"),
...
# Total mean annual hooks per fleet × fishery × period (already computed in diagnostic section)
write_csv(hooks_by_fp,
          here(paste0("output/hooks_by_fleet_period_03g_", date_sfx, ".csv")))

cat("Decomposition tables saved.\n")
```
...
Compare 03g per-capita fishing mortality against known survival rates from the
...
Mapping from 03g age classes to demographic model survival rates:
...
known_survival <- tibble(
  ...
  # Use mean survival across the stages that map to each 03g age class
  s_observed = c(
    0.846,                          # s_Juv
    0.921,                          # s_Imm
...
# Combine DLL + PLL per capita rates from 03g
...
cat("If fishing mortality from 03g exceeds total mortality, natural mortality\n")
...
if (n_impossible > 0) {
  ...
  cat("  → 03g fishing mortality exceeds observed total mortality for these classes.\n")
...
- **Bycatch deaths/yr** = annual bycatch from 03g (DLL + PLL)
...
already include all sources of mortality. Bycatch from 03g should represent
...
cat("Bycatch deaths = 03g estimate (DLL + PLL, mean annual)\n")
...
cat("Bycatch deaths/year:   ", round(sum(mortality_budget$annual_bycatch), 0), "birds (03g estimate)\n")
...
write_csv(validation, here(paste0("output/demographic_validation_03g_", date_sfx, ".csv")))
write_csv(percap_period_combined, here(paste0("output/demographic_validation_by_period_03g_", date_sfx, ".csv")))
write_csv(mortality_budget, here(paste0("output/mortality_budget_03g_", date_sfx, ".csv")))
```
