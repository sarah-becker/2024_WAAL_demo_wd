---
title: "WAAL bycatch — mean annual effort 1990-2010"
...
# Date suffix — must match the date on which script 03f was run
# (both scripts should be run on the same day for filenames to align)
...
Fishing effort, bird distributions, spatial overlap, and BPUE estimation are all handled
in script 03f (spatial catchability approach). Here we load mean annual per-capita bycatch
rates averaged across all four 5-year periods (1990-2010), representing unmitigated bycatch
under typical observed fishing effort across the full study period.

**load per-capita bycatch rates from 03f (mean annual 1990-2010)**
```{r}
# Load period-specific bycatch totals from 03f
byc_period <- read_csv(here(paste0("output/bycatch_by_fishery_period_03f_2026-02-24.csv")), show_col_types = FALSE)

# Load demographic data for population sizes
dem_props <- read_csv(here("data/WAAL_dist_Clay2019/Dem_props copy.csv"),
                      show_col_types = FALSE) %>%
  mutate(Dem_class = str_replace(Dem_class, "J2\\+3", "J2J3"))

# Mean annual bycatch: average the annual rate across all four 5-year periods (1990-2010)
# This represents typical unmitigated annual bycatch under observed fishing effort
byc_mean_annual <- byc_period %>%
  group_by(fishery) %>%
  summarise(across(c(fb, sb, nb, j2j3, imm), ~mean(.x, na.rm = TRUE)), .groups = "drop") %>%
  pivot_longer(cols = c(fb, sb, nb, j2j3, imm), names_to = "age_class", values_to = "annual_byc")

# Mean population size: average across all four Clay periods (1990-2009)
pop_mean <- dem_props %>%
  filter(Period != "1990-2009", Dem_class != "J1") %>%
  group_by(Dem_class) %>%
...
byc_mean_annual <- byc_mean_annual %>%
  left_join(pop_mean %>% select(age_class, Pop), by = "age_class") %>%
  mutate(percap = annual_byc / Pop)

byc_mean_annual %>% print(n = 20)

# Extract per-capita rates by fishery and age class
dem_fm_juv <- byc_mean_annual %>% filter(fishery == "DLL", age_class == "j2j3") %>% pull(percap)
dem_fm_imm <- byc_mean_annual %>% filter(fishery == "DLL", age_class == "imm") %>% pull(percap)
dem_fm_fb  <- byc_mean_annual %>% filter(fishery == "DLL", age_class == "fb") %>% pull(percap)
dem_fm_sb  <- byc_mean_annual %>% filter(fishery == "DLL", age_class == "sb") %>% pull(percap)
dem_fm_nb  <- byc_mean_annual %>% filter(fishery == "DLL", age_class == "nb") %>% pull(percap)

pel_fm_juv <- byc_mean_annual %>% filter(fishery == "PLL", age_class == "j2j3") %>% pull(percap)
pel_fm_imm <- byc_mean_annual %>% filter(fishery == "PLL", age_class == "imm") %>% pull(percap)
pel_fm_fb  <- byc_mean_annual %>% filter(fishery == "PLL", age_class == "fb") %>% pull(percap)
pel_fm_sb  <- byc_mean_annual %>% filter(fishery == "PLL", age_class == "sb") %>% pull(percap)
pel_fm_nb  <- byc_mean_annual %>% filter(fishery == "PLL", age_class == "nb") %>% pull(percap)

# Print summary
cat("=== Per-capita fishing mortality (mean annual 1990-2010 bycatch, mean population) ===\n\n")
cat(sprintf("%-8s %10s %10s %10s\n", "Age", "DLL", "PLL", "Total"))
for (ac in c("j2j3", "imm", "fb", "sb", "nb")) {
  d <- byc_mean_annual %>% filter(fishery == "DLL", age_class == ac) %>% pull(percap)
  p <- byc_mean_annual %>% filter(fishery == "PLL", age_class == ac) %>% pull(percap)
...
```{r}
# Load fleet-level bycatch averaged across all four periods (1990-2010)
fleet_byc <- read_csv(here(paste0("output/bycatch_by_fleet_period_03f_2026-02-24.csv")), show_col_types = FALSE) %>%
  group_by(fishery, fleet) %>%
  summarise(across(c(bi_total, fb, sb, nb, j2j3, imm), ~mean(.x, na.rm = TRUE)), .groups = "drop")
...
coverage_info <- read_csv(here(paste0("output/coverage_info_03f_2026-02-24.csv")), show_col_types = FALSE)

# Classify each fleet's BPUE source (across all periods)
...
p_heatmap_recovery <- ggplot(heatmap_data, aes(x = coverage, y = efficacy, fill = lambda >= 1)) +
  ...
  labs(x = "% bycatch-weighted coverage (fisheries with mitigation)",
       ...
       subtitle = "Lines connect compliance levels within each scenario") +
  coord_cartesian(expand = FALSE) +
  theme_bw() +
  theme(panel.grid = element_blank())
p_heatmap_recovery
...
# Juvenile/immature: wider CIs (harder to estimate, fewer resightings)
# Adult stage-specific: moderate CIs (well-studied but stage distinctions uncertain)
surv_params <- list(
  s_Juv = calc_beta_params(0.846, 0.78, 0.91),    # juvenile — wide, hardest to estimate
  s_Imm = calc_beta_params(0.921, 0.88, 0.96),    # immature — moderate
  s_EF  = calc_beta_params(0.913, 0.87, 0.95),    # experienced failed
  s_ES  = calc_beta_params(0.895, 0.85, 0.94),    # experienced successful
  s_ENB = calc_beta_params(0.943, 0.91, 0.97),    # experienced non-breeding
  s_IF  = calc_beta_params(0.920, 0.88, 0.96),    # inexperienced failed
  s_IS  = calc_beta_params(0.892, 0.85, 0.94),    # inexperienced successful
  s_PF  = calc_beta_params(0.935, 0.90, 0.97)     # sabbatical
)
...
# === BREEDING SUCCESS ===
# Wider CIs than other VRs — breeding success has high interannual variability
# (weather, prey availability, predation) with CV often 10-20%
success_params <- list(
...
diag_imm_redraws <- 0     # times s_Imm was redrawn to satisfy s_Imm < min(adult)
diag_juv_redraws <- 0     # times s_Juv was redrawn to satisfy s_Juv < s_Imm
diag_imm_capped <- 0      # times s_Imm hit 100-redraw fallback cap
...
for (k in seq_along(mitigation_scenarios$efficacy)) {
  ...
  for (j in 1:n_sims) {
    ...
    # Survival: draw adults first, then enforce hierarchy via conditional re-draws
    # Hierarchy: s_Juv < s_Imm < min(adult stages)
    ...
    # Enforce s_Imm < min(adult survival): re-draw until satisfied
    ...
    redraw_count <- 0
    while (sim_s_Imm >= min_adult_s && redraw_count < 100) {
      sim_s_Imm <- rbeta(1, surv_params$s_Imm$alpha, surv_params$s_Imm$beta)
      redraw_count <- redraw_count + 1
    }
    diag_imm_redraws <- diag_imm_redraws + redraw_count
    # Fallback: if still violated after 100 tries, cap just below adult min
    if (sim_s_Imm >= min_adult_s) { sim_s_Imm <- min_adult_s - 0.001; diag_imm_capped <- diag_imm_capped + 1 }

    # Enforce s_Juv < s_Imm: re-draw until satisfied
    sim_s_Juv <- rbeta(1, surv_params$s_Juv$alpha, surv_params$s_Juv$beta)
    redraw_count <- 0
    while (sim_s_Juv >= sim_s_Imm && redraw_count < 100) {
      sim_s_Juv <- rbeta(1, surv_params$s_Juv$alpha, surv_params$s_Juv$beta)
      redraw_count <- redraw_count + 1
    }
    diag_juv_redraws <- diag_juv_redraws + redraw_count
    if (sim_s_Juv >= sim_s_Imm) { sim_s_Juv <- sim_s_Imm - 0.001; diag_juv_capped <- diag_juv_capped + 1 }
    ...
    fm_fb  <- clamp_fm(sim_dem_fm_fb,  sim_pel_fm_fb,  min(1 - sim_s_IF, 1 - sim_s_EF), sim_iuu_p)
    fm_sb  <- clamp_fm(sim_dem_fm_sb,  sim_pel_fm_sb,  min(1 - sim_s_IS, 1 - sim_s_ES), sim_iuu_p)
    fm_nb  <- clamp_fm(sim_dem_fm_nb,  sim_pel_fm_nb,  min(1 - sim_s_ENB, 1 - sim_s_PF), sim_iuu_p)
...
cat(sprintf("s_Imm redraws (s_Imm >= min adult): %d (%.1f per sim)\n", diag_imm_redraws, diag_imm_redraws / total_draws))
cat(sprintf("s_Imm fallback caps (100 redraws exhausted): %d\n", diag_imm_capped))
cat(sprintf("s_Juv redraws (s_Juv >= s_Imm): %d (%.1f per sim)\n", diag_juv_redraws, diag_juv_redraws / total_draws))
  ...
  for (iuu_red_idx in seq_along(fixed_iuu_red_vals)) {
    ...
    for (k in seq_along(mitigation_scenarios$efficacy)) {
      ...
      for (j in 1:n_sims_fixed) {
        ...
        # Enforce s_Imm < min(adult survival)
        min_adult_s <- min(sim_s_EF, sim_s_ES, sim_s_ENB, sim_s_IF, sim_s_IS, sim_s_PF)
        sim_s_Imm <- rbeta(1, surv_params$s_Imm$alpha, surv_params$s_Imm$beta)
        redraw_count <- 0
        while (sim_s_Imm >= min_adult_s && redraw_count < 100) {
        ...
        diag2_imm_redraws <- diag2_imm_redraws + redraw_count
        if (sim_s_Imm >= min_adult_s) { sim_s_Imm <- min_adult_s - 0.001; diag2_imm_capped <- diag2_imm_capped + 1 }
        ...
        if (sim_s_Juv >= sim_s_Imm) { sim_s_Juv <- sim_s_Imm - 0.001; diag2_juv_capped <- diag2_juv_capped + 1 }
        ...
        fm_fb  <- clamp_fm(sim_dem_fm_fb,  sim_pel_fm_fb,  min(1 - sim_s_IF, 1 - sim_s_EF), fixed_iuu_p)
        fm_sb  <- clamp_fm(sim_dem_fm_sb,  sim_pel_fm_sb,  min(1 - sim_s_IS, 1 - sim_s_ES), fixed_iuu_p)
        fm_nb  <- clamp_fm(sim_dem_fm_nb,  sim_pel_fm_nb,  min(1 - sim_s_ENB, 1 - sim_s_PF), fixed_iuu_p)
...
cat(sprintf("s_Imm redraws: %d (%.1f per sim)\n", diag2_imm_redraws, diag2_imm_redraws / total_draws2))
cat(sprintf("s_Imm fallback caps: %d\n", diag2_imm_capped))
...
fig_path <- function(name) here("output/figures", paste0(name, "_mean1990_2010_", date_sfx, ".png"))
