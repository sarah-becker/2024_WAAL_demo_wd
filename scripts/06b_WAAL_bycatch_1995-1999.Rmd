...
**set vital rates (period-specific survival 1995–1999, stage-specific behavioral rates)**
```{r}
# Pooled survival rates (same starting values as 06a; period variation via bycatch data)
s_Juv   <- 0.725  # juvenile survival
s_Adult <- 0.940  # pooled adult survival (applied uniformly to all adult stages)

# Assign pooled adult survival to all adult stage-specific survival variables
s_Imm <- s_Adult; s_EF <- s_Adult; s_ES <- s_Adult
s_ENB <- s_Adult; s_IF <- s_Adult; s_IS <- s_Adult; s_PF <- s_Adult
...
# === SURVIVAL RATES (simplified pooled) ===
# Pooled survival: s_Juv for juveniles, s_Adult for all adult/immature stages
# Behavioral rates (b_*, r_*, k_*) retain full stage-specific structure
surv_params <- list(
  s_Juv   = calc_beta_params(0.725, 0.650, 0.795),  # juvenile survival — wide CI
  s_Adult = calc_beta_params(0.940, 0.900, 0.970)   # pooled adult survival — moderate CI
)
...
for (k in seq_along(mitigation_scenarios$efficacy)) {
  ...
  for (j in 1:n_sims) {
    ...
    # Survival: draw pooled adult first, assign to all adult stages, then draw juvenile
    # Hierarchy: s_Juv < s_Adult
    sim_s_Adult <- rbeta(1, surv_params$s_Adult$alpha, surv_params$s_Adult$beta)
    sim_s_EF  <- sim_s_Adult
    sim_s_ES  <- sim_s_Adult
    sim_s_ENB <- sim_s_Adult
    sim_s_IF  <- sim_s_Adult
    sim_s_IS  <- sim_s_Adult
    sim_s_PF  <- sim_s_Adult
    sim_s_Imm <- sim_s_Adult

    # Enforce s_Juv < s_Adult: re-draw until satisfied
    sim_s_Juv <- rbeta(1, surv_params$s_Juv$alpha, surv_params$s_Juv$beta)
    redraw_count <- 0
    while (sim_s_Juv >= sim_s_Adult && redraw_count < 100) {
      sim_s_Juv <- rbeta(1, surv_params$s_Juv$alpha, surv_params$s_Juv$beta)
      redraw_count <- redraw_count + 1
    }
    diag_juv_redraws <- diag_juv_redraws + redraw_count
    if (sim_s_Juv >= sim_s_Adult) { sim_s_Juv <- sim_s_Adult - 0.001; diag_juv_capped <- diag_juv_capped + 1 }
    ...
    fm_imm <- clamp_fm(sim_dem_fm_imm, sim_pel_fm_imm, (1 - sim_s_Adult), sim_iuu_p)
    fm_fb  <- clamp_fm(sim_dem_fm_fb,  sim_pel_fm_fb,  (1 - sim_s_Adult), sim_iuu_p)
    fm_sb  <- clamp_fm(sim_dem_fm_sb,  sim_pel_fm_sb,  (1 - sim_s_Adult), sim_iuu_p)
    fm_nb  <- clamp_fm(sim_dem_fm_nb,  sim_pel_fm_nb,  (1 - sim_s_Adult), sim_iuu_p)
  ...
  for (iuu_red_idx in seq_along(fixed_iuu_red_vals)) {
    ...
    for (k in seq_along(mitigation_scenarios$efficacy)) {
      ...
      for (j in 1:n_sims_fixed) {
        ...
        if (sim_s_Juv >= sim_s_Adult) { sim_s_Juv <- sim_s_Adult - 0.001; diag2_juv_capped <- diag2_juv_capped + 1 }
        ...
        fm_imm <- clamp_fm(sim_dem_fm_imm, sim_pel_fm_imm, (1 - sim_s_Adult), fixed_iuu_p)
        fm_fb  <- clamp_fm(sim_dem_fm_fb,  sim_pel_fm_fb,  (1 - sim_s_Adult), fixed_iuu_p)
        fm_sb  <- clamp_fm(sim_dem_fm_sb,  sim_pel_fm_sb,  (1 - sim_s_Adult), fixed_iuu_p)
        fm_nb  <- clamp_fm(sim_dem_fm_nb,  sim_pel_fm_nb,  (1 - sim_s_Adult), fixed_iuu_p)
